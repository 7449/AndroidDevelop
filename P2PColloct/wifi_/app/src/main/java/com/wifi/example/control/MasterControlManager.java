...
import com.wifi.example.control.search.MasterSearchManager;
import com.wifi.example.control.websocket.MasterWebSocketManager;
...
public class MasterControlManager implements MasterSearchManager.MasterSearchManagerCallback, MasterWebSocketManager.MasterSocketManagerCallback {
    ...
    public void onFoundNewSlave(String slaveIp, JSONObject slaveInfo) {
        KLog.i(TAG, "found slave " + slaveIp + " info=" + slaveInfo);
    }

    @Override
    public void onFoundSlaves(List<String> slaveIpList) {
        KLog.i(TAG, "slave count = " + slaveIpList.size());
        String msg = "发现从机：\n";
    ...
    public void onServerError(final Exception ex) {
        KLog.i(TAG, "start server error " + ex.getLocalizedMessage());
        handler.post(new Runnable() {
            @Override
            public void run() {
    ...
    public void onClientConnected(WebSocket clientSocket) {
        ...
        KLog.i(TAG, "client connected: " + workSocket.getRemoteAddress().getHostName());
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (controlManagerCallback != null) {
    ...
    public void onClientDisconnect(WebSocket clientSocket) {
        final AsyncNetworkSocket workSocket = (AsyncNetworkSocket) clientSocket.getSocket();
        KLog.i(TAG, "client onClientDisconnect: " + workSocket.getRemoteAddress().getHostName());
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (controlManagerCallback != null)
    ...
    public void onMessageReceive(WebSocket clientSocket, final String message) {
        final AsyncNetworkSocket workSocket = (AsyncNetworkSocket) clientSocket.getSocket();
        KLog.i(TAG, "receive: " + message + "from: " + workSocket.getRemoteAddress().getHostName());
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (controlManagerCallback != null)
    ...
    public void onFileReceive(WebSocket clientSocket, final String filePath) {
        final AsyncNetworkSocket workSocket = (AsyncNetworkSocket) clientSocket.getSocket();
        KLog.i(TAG, "receive: " + filePath + "from: " + workSocket.getRemoteAddress().getHostName());
        handler.post(new Runnable() {
            @Override
            public void run() {
                if (controlManagerCallback != null)
