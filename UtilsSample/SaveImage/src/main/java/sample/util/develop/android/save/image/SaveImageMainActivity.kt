...
import kotlinx.android.synthetic.main.save_image_activity_main.*
...
class SaveImageMainActivity : AppCompatActivity() {
    ...
    private inner class SaveObservable(private val drawingCache: Bitmap?) : Observable.OnSubscribe<String> {
        ...
        override fun call(subscriber: Subscriber<in String>) {
            if (drawingCache == null) {
                ...
                try {
                    val imageFile = File(Environment.getExternalStorageDirectory(), "saveImageview.jpg")
    ...
    private inner class SaveSubscriber : Subscriber<String>() {
        ...
        override fun onError(e: Throwable) {
            ...
            Toast.makeText(applicationContext, "保存失败——> " + e.toString(), Toast.LENGTH_SHORT).show()
        }
